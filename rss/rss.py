...
class RSS(object):
    ...
    @rss.command(pass_context=True, name="cache")
    async def _rss_cache(self, ctx):
        """Show cached feeds."""
        await self.bot.say(self.cache)
        
    @rss.command(pass_context=True, name="maxcache")
    async def _rss_maxcache(self, ctx, value: int):
        """Set max amount of cache that prevents duplicate posts."""      
        self.cache_limit = value
        await self.bot.say(self.cache_limit)
                        ...
                        try:
                            ...
                            if msg is not None and msg not in self.cache[chan_id]:
                                if len(self.cache[chan_id]) >= self.cache_limit:
                                    del self.cache[chan_id][0]
                                self.cache[chan_id].append(msg)
                                await self.bot.send_message(channel, msg)
