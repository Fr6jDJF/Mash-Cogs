...
class Translated:
    ...
    async def _update(self, ctx, languageTo): 
        ...
        await self.bot.say("` Please enter the static system translation value. (=== to cancel)`")
        response = await self.bot.wait_for_message(author=ctx.message.author)
        ...
        await self.bot.say("` Please enter the new static system translation value. (=== to cancel)`")
        response = await self.bot.wait_for_message(author=ctx.message.author)
        response = response.content.strip()
        if response == "===":
            await self.bot.say("cancled translation update")
